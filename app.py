...
def get_news_image(url):
    """실제 뉴스 기사의 대표 썸네일(og:image)을 추출합니다 (구글 로고 무시)."""
    try:
        headers = {
        ...
        res = session.get(url, headers=headers, timeout=6, allow_redirects=True)
        ...
        for _ in range(2):
            if "news.google.com" in res.url or "consent.google.com" in res.url:
                ...
                if meta and 'url=' in meta.get('content', '').lower():
                    ...
                    res = session.get(next_url, headers=headers, timeout=6)
                    continue
                a_tag = soup.find('a', href=True)
                if a_tag and a_tag['href'].startswith('http'):
                    res = session.get(a_tag['href'], headers=headers, timeout=6)
                    continue
                break
                
        # 2. 실제 기사 웹페이지에서 og:image 추출
        soup = BeautifulSoup(res.content, 'html.parser')
        og_img = soup.find('meta', property='og:image')
        
        if og_img and og_img.get('content'):
            img_url = og_img['content']
            # 구글의 기본 로고나 투명 픽셀이 아니면 해당 이미지 반환
            if "news.google.com" not in img_url and "gstatic.com" not in img_url:
                return img_url
    except:
        pass
        
    # 3. 크롤링 실패 시 다양한 뉴스 테마의 고화질 이미지 랜덤 반환
    fallback_images = [
        ...
        "https://images.unsplash.com/photo-1572949645841-094f3a9c4c94?w=400&q=80"
    ]
    ...
    for i, topic in enumerate(final_topics):
        ...
        with col:
            ...
            for j, item in enumerate(feed.entries[:5]):
                # 💡 구글 로고를 뚫고 실제 기사 이미지를 가져오는 함수 적용!
                actual_img_url = get_news_image(item.link)
